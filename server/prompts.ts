...
4. Кросс-функциональная схема (swimlane diagram)
   ...
   - Подробное описание шага (2-3 предложения)
   - Ответственный (конкретная должность)
   - Время выполнения (например, "2 рабочих дня", "60 минут")
   - Подэтапы (3-6 детальных действий с описанием)
   - Информационные системы (для каждой указать название, назначение, действия)
   - Вход (что нужно на старте)
   - Выход (результат/артефакт)
   - Регламенты (список применимых регламентов)
   - Документы (список создаваемых/используемых документов)
   - МОП (Материалы, Оборудование, Персонал):
     * Материалы: детальный список необходимых материалов
     * Оборудование: детальный список необходимого оборудования
     * Персонал: требования к персоналу (квалификация, количество, опыт)
  ...
  "steps": [
    {
      ...
      "responsible": "Конкретная должность",
      "substeps": [
        "Подэтап 1: детальное описание",
        "Подэтап 2: детальное описание",
        "Подэтап 3: детальное описание"
      ],
      "informationSystems": [
        {
          "name": "amoCRM",
          "purpose": "Для чего используется",
          "actions": ["Создать задачу", "Обновить статус"]
        },
        {
          "name": "1С",
          "purpose": "Для чего используется",
          "actions": ["Провести документ"]
        }
      ],
      "mop": {
        "materials": ["Материал 1", "Материал 2"],
        "equipment": ["Оборудование 1", "Оборудование 2"],
        "personnel": ["Специалист 1 (квалификация)", "Специалист 2"]
      },
      "input": "Что нужно на входе",
      "output": "Что получаем на выходе",
      "regulations": ["Регламент 1", "Регламент 2"],

...
### Формат диаграммы Swimlane

Диаграмма строится с **ВЕРТИКАЛЬНЫМИ дорожками** (колонками) по ролям:
- Каждая роль = вертикальная колонка с заголовком сверху
- Блоки располагаются сверху вниз в порядке выполнения
- Связи между блоками идут вертикально (в пределах роли) или горизонтально (между ролями)

### Типы блоков (библиотека Swimlane)

1. **Start** (Запуск) — зелёная пилюля, начало процесса
2. **Action** (Действие) — серый шестиугольник, основной блок действия
3. **Product** (Продукт) — скруглённый прямоугольник, результат/документ
4. **Decision** (Условие) — ромб с вопросом, точка ветвления
5. **Split** (Разделение) — треугольник, параллельные ветки
6. **End** (Завершение) — прямоугольник с двойными линиями

### Параметры действия (только для type=Action)

- **time** — время выполнения (напр. "30 мин", "2 часа", "1 день")
- **document** — документ (напр. "Договор", "Смета", "Акт")
- **database** — система/БД (напр. "CRM", "1С", "Bitrix")
- **stage** — этап воронки (напр. "Новый лид", "В работе")
...
#### 2. Список ролей (вертикальные дорожки swimlanes)

Выведи список всех ролей, которые участвуют в процессе (каждая роль = вертикальная колонка):
...
3. **Название шага** — КРАТКО (3-5 слов), как подпись на блоке.
4. **Роль** — кто отвечает за шаг (определяет вертикальную колонку).
5. **Тип узла** — Start, Action, Product, Decision, Split, End
6. **Вход** — что уже должно быть к началу шага.
7. **Действия** — 3–5 пунктов списком что делает роль.
8. **Выход** — конкретный результат шага.
9. **Параметры** — time, document, database, stage (для Action).
10. **Связи** — предыдущие и следующие шаги (ID).

**ВАЖНО для вертикальной диаграммы:**
- Блоки в одной роли должны иметь последовательный order (сверху вниз)
- Связи между разными ролями идут горизонтально
- Связи внутри одной роли идут вертикально
...
* Названия шагов должны быть КРАТКИМИ (3-5 слов) для отображения в блоках.
